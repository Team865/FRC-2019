...
import ca.warp7.frc2019.helper.config
import ca.warp7.frc2019.helper.followedBy
...
class DriveLinearPIDCopy : TimedRobot() {
    ...
    private val target = 1787.0 // 60 inches

    private val tab = Shuffleboard.getTab("Drive Linear PID")

    val p: NetworkTableEntry = tab.add("P", 0).entry
    val i: NetworkTableEntry = tab.add("I", 0).entry
    val d: NetworkTableEntry = tab.add("D", 0).entry

    val left: NetworkTableEntry = tab.add("Left", 0).entry
    val right: NetworkTableEntry = tab.add("Right", 0).entry
    ...
    override fun robotPeriodic() {
        ...
        left.setDouble(leftMaster.selectedSensorPosition.toDouble())
        right.setDouble(rightMaster.selectedSensorPosition.toDouble())
    }
